...
import LoadFilms from './components/carga/LoadFilm';
...
export default function PageFilmDetail() {
  ...
  return (
    <div className='grid grid-cols-2 gap-x-3 phone:grid-cols-1 tablet:grid-cols-2 place-items-center place-content-center bg-slate-900 border-2 border-slate-400 rounded-2xl p-2 mt-12 mx-5'>
      {!state.Film.director ? <div>Cargando Film...</div> : <FilmsDetail />}

      <div className='grid tablet:grid-cols-1 phone:grid-cols-1 laptop:grid-cols-2 bg-slate-700 gap-4 overflow-auto w-full h-96 p-3 m-3 rounded-2xl place-items-center'>
